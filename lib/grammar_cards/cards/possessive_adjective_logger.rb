...
module GrammarCards
  module Cards
    module PossessiveAdjectiveLogger
      ...
      class LogFile
        def initialize
          @log_data = GrammarCards::Cards::PossessiveAdjectiveLogger.read_log_file
        end

        def nouns_unused_by(structure_record, noun_superset)
          used = []
          if (i = GrammarCards::Cards::PossessiveAdjectiveLogger.index_for(@log_data, structure_record))
            used = @log_data[i][2] || []
          end
          ...
          if diff.empty?  # all nouns have been used
            @log_data[i][2] = []
            GrammarCards::Cards::PossessiveAdjectiveLogger.dump(@log_data)
            noun_superset
          else
            diff
          end
      ...
      extend PossessiveAdjectiveLogger
    end
  end
end
